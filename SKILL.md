...
`/human-expectations on [minutes]` opts into project-wide review; default 30 minutes. First/new-session intake reads full transcripts. `/human-expectations bootstrap` explicitly drains the pending history; an optional transcript path restricts a proof run to that source. `/human-expectations review` processes one batch. `/human-expectations off` stops automatic scheduling and retains the record.

Native file notifications and observational session events mark potential changes. A single deferred callback runs only when the selected interval is due and the foreground agent is idle. Busy work is not polled; the next settled event is the next opportunity. A resumed session catches up once. No source changes means no extraction call. Parsing runs in a worker thread, outside the foreground event loop; model extraction is a separate bounded, tool-free request, not a maintenance turn injected into the conversation. Foreground activity cancels/defer future background extraction. Project-wide leases prevent concurrent reviewers spending quota on the same batch.

A skill alone cannot guarantee wall-clock execution. The bundled extension provides triggers while pi is running; it is not an always-on daemon. Unsupported file-watching paths/platforms rely on session events/resume. Periodic maintenance processes bounded batches; a large backlog remains visible until drained. There is **no zero-overhead guarantee**: static tool/skill definitions occupy prompt space, workers use CPU/memory, and extraction consumes provider quota. No extra foreground turns, tools, prompts, models, thinking settings or UI components are replaced. Separate model-registry calls do not execute foreground provider-request policy hooks; leave automatic review off if a deployment requires those hooks on every request.
